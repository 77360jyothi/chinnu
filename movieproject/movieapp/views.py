...
def register(request):
    if request.method == 'POST':
        ...
        if user is not None:
            login(request, user)
            return redirect('/')  # Redirect to home page after successful registration and login
        else:
            messages.error(request, "An error occurred while logging in after registration.")
